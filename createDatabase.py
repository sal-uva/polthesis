...
pol_database = create_engine('sqlite:///fb_scrapelist.db')

# quit()

file = 'static/data/fbfiles/facebook_scrapelist_v2.csv'
...
for df in pd.read_csv(file, escapechar='\\', encoding='utf-8', engine='python', chunksize=chunksize, iterator=True):
      ...
      df.index += j
      i+=1
      df.to_sql('fbpagedata', pol_database, index=False, if_exists='append')
      j = df.index[-1] + 1
      print(str(i))