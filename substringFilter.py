...
def createHistogram(inputdf, querystring, inputtimeformat, normalised):
	...
	if timeformat == 'days':
		...
		if normalised:
			...
			# li_timestamps = sorted(li_timestamps)
			# for timestamp in li_timestamps:
			# 	print('starting ' + str(li_timeticks[month]))
			# 	print(str(datetime.fromtimestamp(timestamp).strftime("%m")))
			# 	if(str(datetime.fromtimestamp(timestamp).strftime("%m"))) == li_timeticks[month][:2]:
			# 		count_timestamps = count_timestamps + 1
			# 	else:
			# 		normalised_mentions = (count_timestamps / (li_totalcomments[month])) * 100
			# 		print(normalised_mentions)
			# 		print('new month')
	...
	print('Writing results to csv')
	finaldf = pd.DataFrame(columns=['date','count','percentage'])
	finaldf['date'] = li_timeticks
	finaldf['percentage'] = li_normalisedcounts
	finaldf['count'] = li_counts
	finaldf.to_csv('substring_mentions/occurrances_' + querystring + '.csv')
	...
	plt.savefig('../visualisations/' + querystring + '_full.svg')
	#plt.show()

li_querywords = ['blacks','niggers','cuck','whites','gook','thradthot','we must','ethnic','trump','god emperor','stump the trump','bust the trump','feminists','jews']
