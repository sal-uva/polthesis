...
def substringFilter(querystring='all', histogram = False, mintime = 0, maxtime = 0, stringintitle = False, inputtime = 'months', intrumpthreads = False, normalised = True, writetext = False, docsimilarity = False, wordclusters = False, similaritytype = None):
	...
	if intrumpthreads == False:
		...
		if querystring == 'all':
			...
			df = pd.read_sql_query("SELECT timestamp, comment, title, num FROM poldatabase_18_03_2018 WHERE timestamp > ? AND timestamp < ?;", conn, params=[mintime, maxtime])
		#look for string in subject
		elif stringintitle == False:
			df = pd.read_sql_query("SELECT timestamp, comment, title, num FROM poldatabase_18_03_2018 WHERE lower(comment) LIKE ?;", conn, params=['%' + querystring + '%'])
		#look for sting in comment body (default)
		else:
			df = pd.read_sql_query("SELECT timestamp, title, comment, num FROM poldatabase_18_03_2018 WHERE lower(title) LIKE ?;", conn, params=['%' + querystring + '%'])
...
#getTimeSeparatedCsvs(df='substring_mentions/mentions_trump/mentions_trump_all.csv', timesep='weeks')
...
# dec1_2014 = 1417392000
# incrdate = dec1_2014
# while incrdate < 1521417600:
# 	oldinrdate = incrdate
# 	incrdate = (incrdate + 604800) - 1
# 	week_tpl = (oldinrdate,incrdate)
# 	incrdate = (incrdate + 1)
# 	li_times_weeks.append(week_tpl)
...
def getTrumpThreads(querystring='', getdf=True, maketables=False, getMetaInfo=False):
	...
	if maketables:
		...
		cursor.execute("""
					...
					(
						SELECT thread_num, trump_count, comment FROM (
							...
							(
								SELECT thread_num, comment FROM poldatabase_18_03_2018
		...
		cursor.execute("""
					...
					SELECT thread_no, thread_count, trump_count, (trump_count*1.0) / (thread_count*1.0) as trump_density FROM (
						SELECT thread_no, thread_count, trump_count FROM(
							SELECT thread_no, trump_count, count(*) as thread_count FROM poldatabase_18_03_2018
							INNER JOIN trump_threads ON trump_threads.thread_no = poldatabase_18_03_2018.thread_num
							GROUP BY thread_no
		...
		cursor=conn.cursor()
		cursor.execute("""
					DROP TABLE trump_threads_tmp""")
	if getdf:
		df_trumpthreads = pd.read_sql_query("""
					SELECT thread_num, num, subnum, op, timestamp, title, comment, timestamp_expired, media_filename, media_hash, name, trip, sticky, poster_hash, poster_country FROM poldatabase_18_03_2018
					WHERE thread_num IN (
        				SELECT trump_threads.thread_no FROM trump_threads
...
substringFilter(querystring='drumpfkin', histogram=True, intrumpthreads=False)
substringFilter(querystring='trumptard', histogram=True, intrumpthreads=False)
substringFilter(querystring='drumpf', histogram=True, intrumpthreads=False)

# df_trumpthreads = pd.read_csv('trump_threads.csv', encoding='utf-8')

# df_trumpthreads = df_trumpthreads[df_trumpthreads['op'] == 1]
# print(df_trumpthreads['op'])
# df_trumpthreads = df_trumpthreads['title'].loc[30000:37000]
# df_trumpthreads.to_csv('sampled_threads.csv', encoding='utf-8')
# print(len(df_trumpthreads))

# createHistogram.createHistogram(df_trumpthreads)