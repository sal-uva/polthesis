...
=======
		...
		# print('Writing results to csv')
>>>>>>> a40fe6e762cf2978ad8effbb0e617c4ac5c70878
		if '/' in querystring:
...
def getTrumpThreads(querystring='', getdf=True, maketables=False, getMetaInfo=False):
	...
	if getdf:
		df_trumpthreads = pd.read_sql_query("""
					SELECT thread_num, num, op, timestamp, date_full, title, comment, timestamp_expired, media_filename, media_hash, name, trip, sticky, poster_hash, poster_country, date_month, date_week FROM pol_content
					WHERE thread_num IN (
